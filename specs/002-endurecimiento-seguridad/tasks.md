...
- [ ] T009 [FRONT] [US1] Crear `Frontend/pages/admin-login.js` con `adminLoginPage(estado)` y `bindAdminLogin()`: formulario de credencial, mensajes distintos para 401 / 429 / 503 / red caída, todos en español y **sin revelar nada sobre la credencial esperada** (depende de T008)
- [ ] T010 [FRONT] [US1] Registrar `<script src="./pages/admin-login.js?v=13">` en `Frontend/index.html`, **antes** de `app.js`. Saltarse este paso no da error: la página simplemente no existe
- [ ] T011 [FRONT] [US1] En `render()` de `Frontend/app.js`: las cuatro ramas de `/admin` pasan por la puerta —`sin-credencial` → `adminLoginPage()`, `verificando` → aviso de espera, `verificada` → el panel— y acotar el prefijo para que `#/administracion` deje de abrirlo (depende de T008, T009)
- [ ] T012 [FRONT] [US1] Botón de cerrar sesión en la barra lateral de `Frontend/pages/admin.js` que descarta la credencial y vuelve a `sin-credencial`

**Checkpoint**: el panel exige credencial y falla cerrado en las cinco condiciones de fallo.
...
- [ ] T013 [FRONT] [US2] Agregar `escaparHtml(valor)` a `Frontend/utils.js` — **una sola función** que reemplaza `& < > " '`, con el comentario de que todo atributo va entre comillas dobles y de los contextos donde escapar NO alcanza (`href`, `src`, atributo sin comillas) (D1)
- [ ] T014 [FRONT] [US2] Aplicarla en `Frontend/utils.js`: `serviceOptions()` (en el `value` **y** en el texto de cada opción) y `textoServiciosChatbot()` (depende de T013)
- [ ] T015 [P] [FRONT] [US2] Aplicarla en `Frontend/pages/admin.js`: `reservationsTable`, `vehiclesTable`, `messagesTable` y `appointmentsByServiceMarkup` (depende de T013)
- [ ] T016 [P] [FRONT] [US2] Aplicarla en `Frontend/pages/schedule.js`: los `value="${...}"` de los pasos 0, 1 y 2, y el resumen de confirmación (depende de T013)
- [ ] T017 [P] [FRONT] [US2] Borrar `appointmentSurveyTable()` de `Frontend/pages/schedule.js`. Es código muerto con el mismo defecto: se elimina, no se corrige (FR-009)
- [ ] T018 [P] [FRONT] [US2] Repasar `Frontend/pages/` (`home.js`, `contact.js`, `faq.js`, `services.js`, `tarifas.js`) y `Frontend/chatbot.js` buscando cualquier otra interpolación de dato externo sin tratar. Las constantes de `data.js` no cuentan
...
- [ ] T019 [FRONT] [US3] `bindContact()` de `Frontend/pages/contact.js` hace `POST` a `${API_URL}/mensajes` con `Content-Type: application/json` y `{name, email, message}`. Ante fallo —red, 4xx, 5xx, 429— **no** muestra "¡Mensaje Enviado!": avisa en español, ofrece el WhatsApp del CDA y **conserva lo escrito** (FR-011)
- [ ] T020 [FRONT] [US3] `messagesTable` de `Frontend/pages/admin.js` se alimenta de `GET ${API_URL}/mensajes` con `Authorization: Bearer` desde `sessionStorage`, en vez de `storage.get("messages")`. Un 401 devuelve la sesión a `sin-credencial` (depende de T008, T015)
- [ ] T021 [FRONT] [US3] Quitar de `ensureSeed()` en `Frontend/utils.js` la siembra de mensajes de ejemplo con datos personales ficticios (FR-013)
...
- [ ] T022 [FRONT] [US4] Descargar el script de `https://cdn.tailwindcss.com` a `Frontend/assets/vendor/tailwind.js`, con un comentario de cabecera que diga qué es, de dónde salió y en qué fecha
- [ ] T023 [FRONT] [US4] Mover el bloque `tailwind.config` de `Frontend/index.html` a `Frontend/assets/vendor/tailwind-config.js`, cargado **después** de `tailwind.js`
- [ ] T024 [FRONT] [US4] Apuntar `Frontend/index.html` a los dos archivos locales y eliminar la etiqueta del CDN (depende de T022, T023)
- [ ] T025 [FRONT] [US4] Agregar el `<meta http-equiv="Content-Security-Policy">` a `Frontend/index.html` con la política de D3
- [ ] T026 [FRONT] [US4] Enviar desde `Frontend/server.js` la misma política como cabecera, **más `frame-ancestors 'none'`** (que no funciona en un meta), `X-Content-Type-Options: nosniff`, `Referrer-Policy` y `Permissions-Policy`
- [ ] T027 [FRONT] [US4] Verificar en el navegador que `#/servicios` se ve **idéntica** a antes: es la única página que usa Tailwind, así que si algo se rompió, se rompió ahí. Y que ninguna de las siete rutas reporta violaciones de política. Si se queja del bloque `application/ld+json`, poner su hash SHA-256 en `script-src` — **no aflojar la política**
...
- [ ] T028 [P] [BACK] [US5] Ampliar `Backend/src/app.test.ts` con: **`GET /api/mensajes` sin credencial devuelve 401** (la prueba de FR-030), el 429 del limitador de credencial, y el tope por omisión del listado
- [ ] T029 [BACK] [US5] **Verificar a mano que T028 sirve**: comentar `autenticacionAdmin` en `Backend/src/rutas/mensajes.ts`, correr `npm test` y **ver la suite en rojo**; restaurar y ver el verde. Una prueba de regresión que nunca se vio fallar es una suposición (depende de T028)

### Implementación de la Historia 5

- [ ] T030 [P] [BACK] [US5] Crear `Backend/src/middlewares/limitarPeticiones.ts`: fábrica de ventana deslizante en memoria, **clave = SHA-256 truncado de la dirección de red** (nunca la dirección), purga de entradas vencidas, y opción de contar solo los fallos (D4)
- [ ] T031 [BACK] [US5] Aplicar los dos limitadores en `Backend/src/app.ts`: 20 peticiones / 15 min en `POST /api/mensajes` (T021 del backlog de 001) y 10 **fallos** / 15 min en `/api/admin` (depende de T030)
- [ ] T032 [P] [BACK] [US5] Crear `Backend/src/middlewares/registrarAcceso.ts`: una línea por petición con fecha, método, ruta **sin cadena de consulta**, estado y duración. **Prohibido** cuerpo, cabecera de autorización, cadena de consulta y dirección de red (D9, FR-028)
- [ ] T033 [P] [BACK] [US5] Validar el entorno en `Backend/src/config.ts`: `PORT` numérico válido y `CORS_ORIGIN` presente y con forma de URL. Hoy `CORS_ORIGIN` cae en silencio al valor de desarrollo si falta (FR-025)
- [ ] T034 [P] [BACK] [US5] En `Backend/src/middlewares/autenticarAdmin.ts`, rechazar por lista explícita la credencial de ejemplo de la plantilla. Tiene 38 caracteres y hoy **pasa** el mínimo de 16: quien copie `.env.example` sin editarlo obtiene un panel protegido por un secreto publicado en el repo (FR-026)
- [ ] T035 [P] [BACK] [US5] Actualizar `Backend/.env.example` para que quede explícito que ese valor se rechaza a propósito (depende de T034)
- [ ] T036 [P] [BACK] [US5] Tope por omisión de 100 en `validarFiltroMensajes` de `Backend/src/validacion/mensajes.ts`. Hoy, sin `limite`, `GET /api/mensajes` devuelve **todos** los mensajes (FR-024)
- [ ] T037 [BACK] [US5] Capturar `unhandledRejection` y `uncaughtException` en `Backend/src/server.ts`: hoy un fallo fuera del ciclo de una petición tumba el proceso sin dejar rastro (FR-029)
- [ ] T038 [P] [FRONT] [US5] Endurecer `Frontend/server.js`: `try/catch` alrededor de `decodeURIComponent` (hoy `GET /%` **mata el proceso**), `path.resolve` + comparación con `raiz + path.sep` en vez de `startsWith` a secas, lista de denegación para `server.js`, `*.log` y `.vscode/`, y rechazo de métodos distintos de GET/HEAD (FR-021, FR-022)
...
- [ ] T039 [P] [FRONT] `storage.get` de `Frontend/utils.js` con `try/catch` alrededor de `JSON.parse`: hoy un valor corrupto tumba el panel entero, el mismo tipo de caída total que ya se arregló una vez
...
- [ ] T044 [P] [FRONT] Validar el servicio contra el catálogo en `bindQuickAppointment()` de `Frontend/pages/home.js`, que hoy escribe un servicio fijo sin verificarlo (esquiva FR-004 de la funcionalidad 001)
- [ ] T045 [FRONT] Anotar junto a `API_URL` en `Frontend/data.js` que al publicar hay que cambiar **también** el `connect-src` de la política. Es el punto más fácil de olvidar de toda la funcionalidad
...
- **US5 (P5)**: independiente. Su T029 valida retroactivamente que US1 no se pueda revertir.

**US1 y US2 pueden ir en cualquier orden.** El plan sugiere US2 primero cuando un mismo agente
hace las dos, porque el cambio de contexto sale más barato; la spec pone US1 primero porque es
el MVP. Las dos lecturas son válidas: la única restricción dura es US2 antes que US3.

### Paralelismo

- T005 y T006 (backend de US1) contra T008–T012 (frontend de US1): dominios distintos, agentes
  distintos.
- Dentro de US2: T015, T016, T017 y T018 tocan archivos distintos.
- Dentro de US5: T030, T032, T033, T034, T036 y T038 son independientes entre sí.
...
| Agente | Tareas |
|---|---|
| `webcda-backend` | T001–T003, T005–T007, T028–T037 |
| `webcda-frontend` | T008–T027, T038–T046 |
| Verificación en navegador | T004, T027, T047 |
